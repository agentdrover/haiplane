...
| 3 | Выдать implementer-код на задачу N | **дыра** | `POST /api/auth/chat-pair/start` не принимает тела. Нужен контракт с `{kind, task_id}`. Выдача только из `open`. |
| 4 | Redeem | **есть**, расширить | Intake-ответ байт-совместим (`role=human`, `permissions==CHAT_PAIR_PERMS`, `tests/test_chat_pair.py`). Новые поля — опциональные с дефолтами. |
| 5 | Агентская identity | **дыра** | `resolve_session` форсит `role=human` и issuer. Нужны `acting_principal_id`, `kind`, `bound_task_id` на identity. |
| 6 | `POST /api/sessions/register` | **дыра** | Нет в allowlist. Upsert перезаписывает чужой `principal_id`. |
| 7 | Pair-start remote | **дыра, блокер** | `prepare_pair_branch` checkout'ит хост хаба. Режим надо **persist** — git ещё на submit/done/release/CR. |
| 8–11 | Ветка и push в клоне агента | **есть у агента** | Хаб только записывает имя ветки. |
| 12–14 | Submit-for-review, PR, CI | **дыра** на allowlist | PR — только если у проекта настроен origin; иначе явный отказ, не молчание. |
| 15 | Review-verdict | **человек** | Навсегда закрыт для канала. |
| 16–18 | Fix / done | **дыра** на allowlist | `updates` kind=done. Ownership: только holder этой сессии. |
| revoke | По `kind` | **дыра** | Сегодня одно ведро на все сессии issuer. Intake-кнопка убила бы running implementer. |
...
| `docs/agent-context/change-map.md` | Строка implementer **уже есть** — T-docs её обновляет, не добавляет | T-docs |
