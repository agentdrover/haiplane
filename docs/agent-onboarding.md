...
- **Чистый worktree перед `hub_pair_start`.** При грязном worktree pair-start
  вернёт 422 со списком файлов и командой `git stash push -u`. Ничего не
  удаляется: отказ — это и есть защита несохранённой работы. Сначала commit
  или stash, потом повторный pair-start.
