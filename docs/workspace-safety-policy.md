...
Если `HAIPLANE_WORKSPACE_REPO` совпадает с каталогом, открытым в Cursor, коммитьте или прячьте изменения до `hub_pair_start` — иначе старт задачи будет отклонён.
...
При `HAIPLANE_WORKTREE_PER_TASK=1` включается изоляция через `git worktree`:
