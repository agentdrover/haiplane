...
- Review on green CI (#1405): a cross-model review run is bought only when the CI report for the PINNED sha names no `fail` (checks or `validation_status`); `unknown`/`skipped` are not red. The condition sits in `_policy_and_novelty_allow` before the ladder/cascade/ask-again bypass, so none of them skip it; `machine_review_override=require` does. No report in a project whose CI the hub has seen = the order WAITS (one status note), `accept_ci_run_report` places it; past `REVIEW_CI_WAIT_MINUTES` the poller writes one `review_ci_missing` event and orders without CI. A project with no CI report ever is ordered as before. Deferred-order state lives in `events` (kind + `payload.generation`), never in feed text; every event is once per generation. Only an order (a `review_dispatches` row or a report of the generation) settles a deferred order: the outcome of a deferred attempt is read from the database, not from `maybe_dispatch_review`'s False: a dispatcher refusal (policy, novelty, non-convergence) is checked before the attempt and ends it (`review_ci_order_declined`); a #1399 claim left without a row is a blind create and is never retried (`review_ci_order_uncertain`); only a call that did not go through is retried by the sweep, and the `ORDER_ATTEMPTS`-th failure writes `review_ci_order_exhausted`. Those three are final on both paths, sweep and CI-report intake.
