...
- Ветки Dependabot (`dependabot/<экосистема>/<пакет>-<версия>`) бот именует сам;
  префикс в `.github/dependabot.yml` не настраивается, поэтому он внесён в
  allowlist хука `.githooks/pre-push` — иначе локальный push в ветку бота
  режется собственным правилом имён.

## Обновления зависимостей (Dependabot)

- Бот настроен файлом `.github/dependabot.yml`: экосистема `uv`, каталог `/`,
  `target-branch: develop`, расписание — еженедельно, потолок одновременно
  открытых PR назван числом (`open-pull-requests-limit: 3`).
- **Бот целится в `develop`, а не в `main`.** `main` — релизная ветка: любой
  merge в неё запускает job `deploy` («Deploy to server»), то есть приём PR
  бота в `main` равнозначен выкату в прод мимо `develop`, задачи и ревью, и
  уводит `main` вперёд `develop`. До появления этого файла бот целился в ветку
  по умолчанию (`main`) — так возник PR #404 (22.09.2026), который нельзя было
  принять.
- PR бота проходят обычный CI и обычный путь в `develop`. Автоматического
  приёма по зелёному CI нет — это отдельное решение владельца.
