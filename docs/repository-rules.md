...
- **Репозиторий проекта получает workflow при провижининге** (#476). Шаблоны
  лежат в `hub/workflow_templates/` (`ci.yml`, `stale.yml`) с плейсхолдерами
  ...
  - **Раскладка только в репозиторий, где своих workflow нет вовсе.** Тот, у
    ...
    пайплайн. Это же правило держит репозиторий самого хаба вне раскладки: в
    нём лежит `ci.yml`.
  - **Ничего не угадывается.** В шаблоне CI нет команды сборки: выдуманная
    команда превратила бы `ci_absent` в `ci_failed`, а красный job гейт держит
    как блокер и тратит на него fix-циклы, тогда как честное молчание читается
    как unknown и уходит к человеку. Команду прогона AC проект объявляет сам —
    ключ `ci_runner` в `gate_policy`.
  - **Провижининг не падает из-за раскладки.** Итог — предложение в
    `provision_detail`; неудавшийся push откатывает собственный коммит, чтобы
    клон не остался впереди своего remote.
