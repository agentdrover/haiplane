...
# Третье состояние — и оно не про расчёт, а про ЧТЕНИЕ записи: запись,
# сделанная до #1169, несёт только имя и версию. «Дифа в записи нет» и «диф, в
# котором ничего не изменилось» — разные ответы, и показывать второй вместо
# первого значит утверждать факт, которого нет. Переезд хранилища (#1253) это
# состояние сохранил: миграция перенесла такие записи из ленты на строку
# версии как есть, вместе с отсутствующим в них дифом.
...
# ``skill_activated`` вовсе, и обе версии реестра хаба активны без единой
# записи. Разница с ``BASELINE_UNRECORDED`` не косметическая: там запись есть
...
# квадратичен, и ускорить его нельзя: такова его природа. Замер прежней
# редакции ``summarize_change`` на входе «строки переставлены через одну»
# (b = a[::2] + a[1::2]), интерпретатор проекта, CPython 3.12.13:
#
#     строк  1000,  17 КБ →   0.04 с
#     строк  6000, 104 КБ →   1.54 с
#     строк 16000, 286 КБ →  10.67 с
#
...
# И это ещё половина счёта: ``_skill_publish_views`` зовёт следом
# ``unified_diff``, который делает ту же работу второй раз — вместе 20.62 с.
...
# и общего хвоста. Число выбрано замером, а не на глаз (CPython 3.12.13; время
# приведено за оба вызова сразу — страница зовёт и сводку, и unified diff):
#
#   вход                                клеток    вместе
#   перестановка через одну,  1000 строк   1.0M    0.08 с
#   перестановка через одну,  2000 строк   4.0M    0.31 с   ← потолок здесь
#   перестановка через одну,  6000 строк  36.0M    2.74 с
#   перестановка через одну, 16000 строк 255.9M   20.62 с
#
# То есть потолок держит худший случай в пределах трети секунды вместо двадцати
# секунд. Оценка НАМЕРЕННО грубая и в безопасную сторону: те же 4.0M клеток на
# полностью переписанном файле в 2000 строк и 36.0M на файле из коротких
...
# человек читает словами, а двадцатисекундная пауза съедает рабочий процесс.
...
def _trimmed(a: list[str], b: list[str]) -> tuple[int, int]:
    ...
    после отсечения превращается в почти пустую задачу. Замер: правка блоком в
    50 строк внутри файла на 20000 строк (634 КБ) — 250 500 клеток и 0.03 с на
    оба вызова, то есть намного ниже потолка.
    """
    head = 0
...
def publication_payload(
    *,
    ...
    """Запись о публикации, одинаковая на всех трёх путях.

    Одна и та же строка ложится в ДВА места (#1253): в событие
    ``skill_activated`` — уведомление, которое человек видит сразу и которое
    чистка ленты унесёт через две недели, — и в ``skills.publication_record``,
    где она и переживает эту чистку. Считать её дважды нельзя: вызывающий
    строит payload один раз и отдаёт одну строку обоим.
