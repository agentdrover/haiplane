...
class GitHubForge:
    ...
    #: Мерж делается с ``--squash`` (см. merge_pr ниже), то есть базовая ветка
    #: получает НОВЫЙ коммит, а сдаточный не становится её предком никогда
    #: (#1214). Родословная здесь судить не может — ни в одну сторону.
    merge_preserves_ancestry = False
