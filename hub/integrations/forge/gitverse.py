...
class GitVerseForge:
    ...
    #: А раз сливает локальный git, то делает он это через ``merge --no-ff``
    #: (git_ops.merge_pr_by_push): головной коммит ветки остаётся в истории
    #: базы. Родословная здесь судить МОЖЕТ, и «не предок» тут — настоящее
    #: «работы в базовой ветке нет» (#1214).
