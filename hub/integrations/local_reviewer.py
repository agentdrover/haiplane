...
async def run_review(
    ...
    *,
    ...
    ``prompt_at_slot`` — последнее слово о промте, и зовут его ОДИН раз:
    слот уже взят, процесса ещё нет. Промт, собранный ЗАРАНЕЕ, к этому
    моменту может устареть, и не по мелочи: он несёт одноразовый код доступа
    ревьюера к хабу, который живёт ``CHAT_PAIR_CODE_SECONDS``, а ждать здесь
    можно всё чужое ревью — десятки минут. Готовить это заранее нельзя ровно
    потому, что «заранее» и есть источник дефекта (#1208, находки ревьюера
    Codex 11.09.2026 на ff5b518 и c6af952).
