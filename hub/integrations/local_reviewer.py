...
# оставлять CLI прямым потомком хаба, ЛИБО иметь СОБСТВЕННЫЙ срок жизни, НЕ
# ПЕРЕЖИВАЮЩИЙ хабский. Первая редакция требовала только потомка — и это
# требование недостижимо для контейнеров: ``podman run`` переживает убийство
# группы процессов (измерено на проде 08.09.2026: kill -KILL по группе, через
# 7 с контейнер «Up»), потому что conmon намеренно отсоединён от клиента
# podman. Ровно то же делает ``systemd-run`` без ``--scope``: родителем
# становится PID 1.
#
...
#
# ОТСОЕДИНЁННЫЙ ЗАПУСК не даёт НИ ТОГО, НИ ДРУГОГО, сколько бы ни стояло в
# ``--timeout`` (найдено ревьюером Codex 10.09.2026; воспроизведено на
# 5b7b813: ``podman run -d --timeout 1800 img`` -> ``[]``). ``-d`` заставляет
# клиента podman вернуть управление НЕМЕДЛЕННО: хаб увидит завершившийся
# процесс, закроет прогон как законченный и может снести его каталог, а
# ревьюер в контейнере продолжит работать и пришлёт отчёт по закрытому
# прогону. Срок жизни у контейнера при этом есть — и он бесполезен, потому
# что снимать уже нечего. Протокол хаба вообще построен на ПЕРЕДНЕМ ПЛАНЕ:
# промт уходит в stdin, ответ читается из stdout, и с ``-d`` не работает ни
# то, ни другое. Поэтому отсоединение отвергается ПО ФЛАГУ, а не по сроку.
#
# СОБСТВЕННЫЙ СРОК НЕ МОЖЕТ БЫТЬ БОЛЬШЕ ХАБСКОГО (та же находка, второй
# случай: ``--timeout 3600`` при ``LOCAL_REVIEW_TIMEOUT_SEC = 1800`` ->
# ``[]``). Смысл собственного срока один — чтобы контейнер УМЕР, когда хаб
# счёл прогон зависшим и убил клиента. Срок вдвое длиннее хабского этого не
# даёт: контейнер переживёт снятие прогона ровно так же, как без ``--timeout``
# вовсе, только тише. Обратная сторона — срок КОРОЧЕ хабского — дырой не
# является: контейнер умрёт сам, клиент podman вернётся, хаб закроет прогон
# по коду возврата и назовёт причину. Она лишь укорачивает время ревьюеру,
# поэтому страж требует «не больше», а документ РЕКОМЕНДУЕТ равенство. Раньше
# документ советовал «не меньше» — то есть ровно то, что страж обязан
# отвергать; согласие текста с кодом держится тестами, а не чтением.
#
...
_PODMAN_DETACH_HINT = (
    "LOCAL_REVIEW_SANDBOX: podman run «{flag}» запускает контейнер "
    "ОТСОЕДИНЁННЫМ, и --timeout этого не лечит: клиент podman вернёт "
    "управление немедленно, хаб сочтёт прогон законченным и может снести его "
    "каталог, пока ревьюер внутри контейнера ещё работает — а потом пришлёт "
    "отчёт по закрытому прогону. К тому же промт уходит контейнеру в stdin, а "
    "ответ читается из stdout: отсоединённый запуск не даёт ни того, ни "
    "другого. Уберите «{flag}» — контейнер обязан идти на переднем плане — "
...
_PODMAN_UNKNOWN_FLAG_HINT = (
    ...
    "{fix} — либо уберите флаг из строки песочницы, спрятав podman "
    "внутрь враппера-скрипта, как советует deploy/LOCAL-REVIEW.md"
)
# Починка зависит от ФОРМЫ флага, и одна на обе не годится: длинному имени
# «=значение» дописывается к нему самому, а короткому токену — НЕТ. На
# ``-p80:80`` совет «напишите -p80:80=<значение>» называл форму, которой у
# podman не существует вовсе: pflag разбирает ``-p80:80=x`` как значение
# ``80:80=x`` у ``-p``. Отказ, который советует невозможное, оператору не
# помогает — а именно называние починки этот страж и обещает.
_LONG_FLAG_FIX = (
    "Напишите флаг в форме {flag}=<значение> — она однозначна и от списков не зависит"
)
_SHORT_FLAG_FIX = (
    "Дописать «=<значение>» к «{flag}» нельзя: у короткого флага значение "
    "может быть слипшимся с буквой, и где кончается флаг, из токена не "
    "видно. Запишите его ДЛИННЫМ именем в форме --флаг=значение — она "
    "однозначна и от списков не зависит"
)


def _unknown_flag_fix(flag: str) -> str:
    """Как чинить нераспознанный флаг — по его ФОРМЕ, а не одним шаблоном."""
    return (_LONG_FLAG_FIX if flag.startswith("--") else _SHORT_FLAG_FIX).format(
        flag=flag
    )
...
# Значения, при которых булев флаг pflag ВЫКЛЮЧЕН. Список закрытый и по
# имени: pflag разбирает значение булева флага через strconv.ParseBool, а тот
# знает ровно «1, t, T, TRUE, true, True» и «0, f, F, FALSE, false, False» —
# на всём остальном он возвращает ошибку, и podman не запустится вовсе.
_PFLAG_FALSE = frozenset({"0", "f", "F", "FALSE", "false", "False"})


def _detaching_flag(flags: list[tuple[str, str | None]]) -> str:
    """Имя ДЕЙСТВУЮЩЕГО флага отсоединения среди флагов run; «» — его нет.

    Отсоединение записывается тремя способами, и знать один значит пропускать
    два: длинным именем ``--detach``, коротким ``-d`` и буквой ``d`` внутри
    слипшегося пучка (``-itd``). Все три разбираются pflag как один и тот же
    булев флаг, поэтому и здесь они судятся вместе — по ПОСЛЕДНЕМУ вхождению
    (_effective), как всё в этом модуле.

    ``--detach=false`` и ``-d=false`` отсоединения не включают, и отвергать
    их значило бы отказывать на рабочей строке. А вот у ПУЧКА значение
    достаётся последней букве (``-itd=false`` выключает ``d``, ``-idt=false``
    — ``t``), и какой именно — из одного токена не видно: пучок судится как
    отсоединение независимо от значения. Ошибка здесь идёт в сторону ОТКАЗА,
    и починка названа — записать флаг отдельным токеном.
    """
    occurrences = [
        (name, value)
        ...
        or (
            ...
            and "d" in name[1:]
            and _is_valueless_run_flag(name)
        )
    ]
    effective = _effective(occurrences)
    if effective is _FLAG_ABSENT:
        return ""
    name, value = effective
    if name in ("--detach", "-d") and value is not None and value in _PFLAG_FALSE:
        return ""
    return name
...
def detaching_sandbox(limit: int | None = None) -> list[str]:
    ...
    # Отсоединение судится ОТДЕЛЬНО от срока и НЕЗАВИСИМО от того, удался ли
    # разбор до конца: ``-d`` — это факт, увиденный в окне флагов запуска, и
    # никакой ``--timeout`` его не отменяет. Ставить эту проверку внутрь
    # ветки срока значило бы повторить ошибку пятого круга, когда починка
    # одной находки сделала вторую недостижимой на её собственном входе.
    if podman_flags is not None:
        detach = _detaching_flag(podman_flags.flags)
        if detach:
            reasons.append(_PODMAN_DETACH_HINT.format(flag=detach))
    if podman_flags is not None and podman_flags.unknown:
        ...
        reasons.append(
            _PODMAN_UNKNOWN_FLAG_HINT.format(
                flag=podman_flags.unknown,
                fix=_unknown_flag_fix(podman_flags.unknown),
            )
        )
