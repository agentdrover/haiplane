...
def _validate_count(policy: dict[str, Any], key: str) -> None:
    """Счётный ключ — целое ≥ 0 или ничего (#1414 deep_daily_cap, #1416
    small_delta_lines).

    ``True`` — тоже int в Python, и «потолок true» читался бы как 1; строка
    «2» выглядела бы числом, не будучи им для читателя. Обе — отказ.
    """
    if key not in policy:
        return
    value = policy[key]
    if isinstance(value, bool) or not isinstance(value, int) or value < 0:
        raise ValueError(f"gate_policy {key} must be an integer >= 0, got: {value!r}")
...
class DeliveryAcknowledgement(BaseModel):
    ...
    Обрезка ДО проверки длины, а не после, и это не мелочь: без неё три
    пробела проходили порог, добирались до хранилища и получали 404 «строки
    нет» — отказ, называющий не ту причину. А «  a» и вовсе записывало
    причину в один символ, обойдя порог пробелами. Форма в инбоксе обрезала
    сама, API — нет, и два входа в один глагол вели себя по-разному.
    """
    ...
    def _trimmed(cls, value: str) -> str:
        ...
        if len(trimmed) < 3:
            raise ValueError(
                "причина признания — предложение, а не пробелы: "
                "без неё это выключатель, а не решение"
            )
        return trimmed
...
STEWARD_CLOSURE_TYPES: tuple[str, ...] = (
    ...
    "human_disposition",
    "out_of_scope_linked",
    # #1170: судьба НЕРАЗРЕШЁННОЙ находки, названная её автором словарём
    # #1085 и записанная хабом на пересдаче. Стюард такую находку не судит —
    # адъюдикаторы её не рассудили, и его мнение было бы третьим, — он
    # проверяет, что автор её рассудил. Применим только к разделу
    # ``unresolved``: опись в steward_apply.CLOSURE_SECTIONS.
    "author_outcome",
)
