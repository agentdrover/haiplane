...
async def _poll_running_tasks(app: FastAPI) -> None:
    ...
    never reaches ``_task_isolation`` — is swallowed by name in ``_poll_once``,
    so the remaining sweeps of this tick still run (#1069). Before that, a
    single timed-out call in the second sweep cost the eighteen after it, every
    tick until the cause cleared.

    The ``try`` here is what is left of the old catch-all: a backstop for
    anything outside the sweeps, proving the loop itself never dies.
...
async def _poll_once(db) -> None:
    """Один проход конвейера: весь ``SWEEPS`` по порядку, каждый — сам за себя.

    Упавший проход называется в журнале по имени и не отменяет следующие
    (#1069). Порядок несущий — см. докстроку ``SWEEPS``.
    """
    for sweep in SWEEPS:
        try:
            await sweep.run(db)
