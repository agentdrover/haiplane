...
#: Имена, которые подписка разрешает ЗАПУСКАТЬ (#1237). Один список на всех,
#: кто выбирает модель у провайдера, — потому что расхождение уже случилось и
#: стоило эпику #994 всех его суждений.
#:
#: СПОСОБ ПРОВЕРКИ — ПОПЫТКА СОЗДАНИЯ АГЕНТА (POST /v1/agents), НЕ list_models.
#: Дата: 28.08.2026 (#1036), запуском каждого имени. Наличие имени в каталоге
#: доказательством НЕ считается и в этот список не пускает: именно так сюда
#: попали gpt-5.3-codex и gemini-3.1-pro — они есть в каталоге и отвечают
#: HTTP 400 usage_limit_exceeded при создании, то есть были не запасными
#: судьями, а гарантированным отказом.
#:
#: Проверка НЕ ПЕРЕСНЯТА на 09.09.2026: живая попытка требует ключа
#: провайдера, который лежит только на прод-хосте. Дата выше — возраст
#: свидетельства, а не дата чтения кода; устаревает она молча, поэтому
#: пересъёмка стоит в outcome_revisit_condition задачи #1237.
...
#
# composer-2.5, а не gpt-5.3-codex (#1237): в подписке три запускаемых имени
# и всего два семейства — grok и composer. Ревьюер по своему списку берёт
# grok-4.6 первым, исполнитель по факту claude, поэтому третьим семейством
# может оказаться только composer. Это не выбор из многих, а единственное
# имя, которое одновременно запускается и проходит гейт разнообразия;
# случай «третьего семейства не нашлось» теперь называется вслух, а не
# молчит (steward_shadow.REFUSED_NO_LAUNCHABLE_JUDGE).
STEWARD_MODEL = env_get("STEWARD_MODEL", "composer-2.5")
...
#
# Имена берутся ТОЛЬКО из SUBSCRIPTION_LAUNCHABLE_MODELS — там же записано,
# чем и когда проверено. Обе оставшиеся замены из семейства grok: на обычной
# сдаче (исполнитель claude, ревьюер grok-4.6) гейт их отклонит, и годной
# замены не останется. Это честное состояние подписки, а не забытый список:
# отказ по исчерпанию называет причину и не сжигает слот молча.
STEWARD_MODEL_FALLBACKS = tuple(
    m.strip()
    for m in env_get("STEWARD_MODEL_FALLBACKS", "grok-4.6,grok-4.5").split(",")
    if m.strip()
)
