...
#: Минимальная длина каждого поля доказательства. Порог, а не детектор лжи, и
#: называть его иначе значит обещать больше, чем код делает. Он ловит форму
#: штампа: «проверено» (9) и «всё хорошо» (10) не проходят ни порознь, ни
#: россыпью по двум полям. Развёрнутое доказательство — «git show 19ee3f6
#: --stat» и «файл присутствует, AC-тест зелёный» — проходит без усилий.
#: Судит запись читатель, которому она приписана поимённо; схема отвечает
#: только за то, чтобы судить БЫЛО ЧТО.
...
async def record_delivery_observation(
    ...
    *,
    ...
    prior = await repo.record_delivery_observation(
        db, task_id, by=by, probe=probe, evidence=evidence, sha=sha
    )
    # Строку только что прочитали, но между чтением и записью её мог удалить
    # каскад от удаления задачи. Тогда закрывать нечего — и это тот же отказ,
    # что и выше, а не падение.
    if prior is None:
        raise ObservationRefused(
            "no_discrepancy",
            f"по задаче #{task_id} расхождения доставки не записано",
