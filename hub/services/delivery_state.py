...
async def record_delivery_observation(
    ...
    *,
    ...
    # ней что-то изменилось, и предикат состояния поймал ровно это (находка
    # 493a0ee9): либо строку унёс каскад от удаления задачи, либо свип успел
    # зафиксировать ответ ожившего источника. Оба случая — отказ, а не
    # падение и не тихая запись поверх чужого факта. Перечитываем, чтобы
    # назвать читателю тот, который случился на самом деле.
    if prior is None:
