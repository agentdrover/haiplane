...
async def blocker_delivery_cached(db: Any, blocker: dict[str, Any]) -> dict[str, Any]:
    ...
    2. записанный ответ свипа (#897) — та самая строка, которую читает
       ...
       вопрос платят вызовом провайдера. Squash-мерж виден только отсюда:
       ancestry он не сохраняет, так что ``is_ancestor`` о нём говорит «нет»;
    3. если не записано ничего — остаётся ответ шага 1.
