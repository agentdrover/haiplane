...
async def refine_tasks_bulk(
    ...
    #
    # Автопилот DoR-апрува (#1164) едет тем же общим помощником, и по той же
    # причине: он висел на ``recalc_readiness_inline``, поэтому один и тот же
    # патч давал разный гейт — через /refine драфт открывался, через
    # /refine-bulk оставался в draft. Дописывать сюда ВТОРОЙ вызов
    # ``maybe_auto_approve`` было нельзя: это воспроизвело бы ровно тот
    # дефект, из которого задача выросла, на следующем пути записи. Вызов
    # остаётся один и стоит в ``_persist_readiness_and_revision``.
    #
    # ОКНО МЕЖДУ ФАЗАМИ, ИЗМЕРЕННОЕ, А НЕ ВЫЧИТАННОЕ (находка отчёта #313).
    # Патч уже на диске, а этот проход ещё не сделан — рестарт службы или
    # ошибка расчёта оставляют пакет без автопилота. Что при этом бывает,
    # проверено сценарием в
    # ``test_a_half_finished_batch_approves_nobody_and_stays_recoverable``:
    # частично одобренного пакета не возникает (транзакция откатывает и тех,
    # до кого проход дошёл), драфты остаются в draft — то есть отказ смотрит
    # в сторону человека, — и повтор того же запроса доводит пакет до конца.
    # Чтение карточки между падением и повтором ничего не закрепляет: оно
    # чинит поля готовности и не трогает гейт.
...
async def _persist_readiness_fields(
    ...
    БАМП ЗДЕСЬ БЫЛ БЫ ВРЕДЕН, А НЕ ПРОСТО ИЗБЫТОЧЕН (#1166). У задачи,
    которую после появления колонки ни разу не правили,
    ``statement_fingerprint`` пуст, а пустая строка не равна sha256 ни от
    чего — на такой строке бамп ОБЪЯВИЛ БЫ ревизию, хотя постановку никто
    не трогал. Чтение и правда постановку не меняет; базиса, с которым
    сравнивать, у неё пока нет. Цену видно этажом дальше: ``get_readiness``
    пишет в строку по ходу чтения, так что просмотр небазированной карточки
    дал бы 0 → 1 и потратил слот ``kind='dor'`` на ревизию, которой не было.

    Держит это разделение тест, а не абзац выше:
    ``test_viewing_an_unbaselined_row_buys_no_revision`` в
    ``tests/test_statement_generation.py`` читает готовность на строке БЕЗ
    базиса, и перенос бампа сюда роняет именно его. Соседний
    ``test_reading_readiness_is_not_a_statement_edit`` начинается с refine,
    отпечаток у него уже снят, и переноса он не заметит — он ловит другую
    реализацию, «расти на каждом вызове».
    """
...
async def _persist_readiness_and_revision(
    ...
    """Записать готовность, двинуть поколение постановки и спросить автопилот.

    Единственное место, где счётчик ревизий увеличивается (#1156) и где
    спрашивают DoR-автопилот (#1164). Мест вызова два, потому что путей
    записи два: одиночный refine и мутации AC и рисков считают готовность
    внутри своей транзакции (``recalc_readiness_inline``), массовый refine —
    пакетом после коммита. Правило при этом одно, и добавить путь записи, не
    задев его, теперь нельзя: готовность без ревизии и без автопилота здесь
    не записывается.

    ПОЧЕМУ АВТОПИЛОТ ЗДЕСЬ, А НЕ У ОБОИХ ВЫЗЫВАЮЩИХ (#1164). Он стоял в
    ``recalc_readiness_inline``, куда массовый refine не заходит, и один и
    тот же DoR-патч давал разный гейт: через /refine драфт открывался,
    через /refine-bulk оставался в draft. Второй вызов рядом с первым
    закрыл бы сегодняшнее расхождение и завёл завтрашнее — на третьем пути
    записи. Поэтому вызов ровно один, и стоит он на общем помощнике.

    И ПОЧЕМУ НЕ ЭТАЖОМ НИЖЕ, в ``_persist_readiness_fields``: ту зовёт ещё
    и ленивая починка в ``get_readiness``, то есть обычное ЧТЕНИЕ карточки.
    Автопилот там открывал бы драфты по факту просмотра.

    Держит этот этаж ТЕСТ, а не абзац выше — как и у соседа снизу.
    ``test_reading_a_card_never_opens_a_draft`` в
    ``tests/test_auto_approve.py`` читает готовность на строке с
    несохранёнными полями (иначе ленивая починка не срабатывает и разницы
    не видно) и падает ровно на переносе вызова вниз. Абзац этого не умел:
    отчёт ревью #313 указал, что перенос оставлял ВСЮ сюиту зелёной, и
    измерение это подтвердило — 3475 passed под мутацией.

    Порядок важен: сначала готовность, потом отпечаток, и только потом
    автопилот. Счёт готовности и ``dor_passed`` в отпечаток не входят, но
    ложатся на ту же строку, и писать их после снятия отпечатка значило бы
    сравнивать со строкой, которой уже нет; а автопилот читает
    ``dor_passed`` из строки и переводит её в open — после него сравнивать
    было бы уже не с чем.
    """
    ...
    # Автоодобрение низкого класса (#584) на ВСЕХ путях записи (#1164).
    # Условие здесь — то же, что стояло у одиночного вызова: автопилот
    # спрашивают только у готового драфта. Все поштучные проверки (класс,
    # оба потолка, ladder, dor=auto проекта, глобальный выключатель) живут
    # внутри и на пакет не смотрят — элемент пакета проходит ровно те же,
    # что и одиночный. С выключателем off (значение по умолчанию) это
    # no-op, и человеческий гейт стоит как прежде.
    if report.dor_passed:
...
async def recalc_readiness_inline(
    ...
    #
    # Счётчик — НЕ под условием dor_passed, в отличие от автопилота, который
    # стоит рядом с ним в том же помощнике. Драфт, не прошедший DoR, правят
    # чаще прошедшего, и именно эти правки автор делает по замечаниям
    # стюарда; счётчик, молчащий до готовности, не заметил бы ровно тот
    # цикл, ради которого он заведён.
    # Автопилот DoR-апрува (#584) тоже внутри общего помощника, и по той же
    # причине (#1164): пока он висел здесь, массовый refine проходил мимо
    # него, и включённая владельцем политика зависела от того, каким HTTP-
    # вызовом драфт довели до готовности.
