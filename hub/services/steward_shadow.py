...
# Захват, оставленный мёртвым процессом и снятый при подъёме (#1195).
# Своё слово, а не steward_run_refused: отказ означает «не смогли
# начать», а здесь начать МОГЛИ и, судя по обмену одноразового кода,
# начали — просто некому стало записать чем.
EVENT_CLAIM_RECOVERED = "steward_claim_recovered"
...
#: Куда прогон кладёт свой допуск. Домашний каталог, а не /tmp: вне рабочего
#: дерева (иначе короткий допуск уедет в артефакты прогона), но и не в общем
#: каталоге, где предсказуемое имя — приглашение подменить файл симлинком.
#: Тильда, а не $HOME, намеренно: правило «ни одна команда не полагается на
#: переменную» остаётся без исключений, которые потом придётся объяснять.
CREDENTIAL_PATH = "~/.haiplane-steward.credential"


def delivery_commands(task_id: int, code: str, base_url: str) -> tuple[str, ...]:
    """Команды, которые прогон выполняет, — каждая самодостаточна (#1194).

    Прежняя форма велела «сохранить токен в переменную», а дальше все команды
    ссылались на ``$TOKEN``. Наблюдено 06.09.2026: обмен кода проходил, сессия
    была жива, а СЛЕДУЮЩИЙ ЖЕ запрос получал 401 — за день шесть заказов и
    ноль суждений. Почему именно допуск не доезжал, журнал не различает:
    переменная не пережила переход к следующей команде, либо токен вовсе не
    был захвачен из ответа. Обе ветки — один дефект, и обе лечит одно
    правило: команде, которой нужен токен, велено добывать его самой.

    Поэтому здесь список, а не сплошной текст. Прозу читает агент, а
    выполняются ровно эти строки — тест гоняет их, а не пересказ.

    ``umask 077`` стоит ДО перенаправления: файл рождается с режимом 600, без
...
def delivery_block(task_id: int, code: str, base_url: str) -> str:
    ...
    if not code or not base_url:
        return ""
    redeem, check, evidence, judgement = delivery_commands(task_id, code, base_url)
    return (
        ...
        "его сразу:\n"
        f"  {redeem}\n"
        "Токен ушёл в файл и в вывод не попал. НЕ переноси его в переменную "
        "оболочки: каждая команда ниже берёт его из файла сама, потому что "
        "переменная не переживает переход к следующей команде — на этом уже "
        "потерян целый день суждений (#1194).\n"
        "Убедись, что файл непустой (печатается РАЗМЕР, не содержимое):\n"
        f"  {check}\n"
        "Пакет доказательств:\n"
        f"  {evidence}\n"
...
async def recover_dead_process_claims(db: aiosqlite.Connection) -> int:
    """Снять захваты, оставленные мёртвым процессом, и назвать их вслух (#1195).

    ИЗМЕРЕННЫЙ СЛУЧАЙ. Задача #1190, поколение 1: заказ размещён в 08:49:05
    UTC, служба перезапущена в 08:49:07, одноразовый код прогона обменян на
    токен в 08:49:20. Тринадцать тиков поллера прошли мимо, и в 08:55:47 слот
    закрыли чужим вердиктом со строкой ``agent_id='pending:6'``.

    Почему мимо: ``start_run`` захватывает слот ДО вызова провайдера — порядок
    верный, он против оплаты раньше захвата (#1075, #421), — а
    :func:`_open_orders_without_runs` отбирает заказы по ``agent_id=''``.
    Помеченный слот в выборку не попадает никогда, и восстановления после
    падения внутри этого окна у механизма не было.

    ПРИЗНАК — СТАРТ ПРОЦЕССА, А НЕ ВОЗРАСТ МЕТКИ. Возврат по таймеру «метка
    старше N минут» был бы гаданием о том, жив ли ещё тот вызов. Процесс,
    который только что поднялся, не может иметь СВОИХ незавершённых вызовов к
    провайдеру, значит любая метка захвата в его базе оставлена мёртвым
    процессом. Это точное утверждение, а не эвристика — и держится оно на
    одном допущении: хаб работает ОДНИМ процессом (проверено наблюдением на
    проде 06.09.2026). Второй воркер сделал бы признак неверным, и тогда
    восстановление придётся привязывать к идентификатору процесса.

    ТОЛЬКО ОТКРЫТЫЕ. Восстановление возвращает в работу недоделанное, а не
    оживляет решённое: у закрытого заказа метка тоже может остаться, и снять
    её значило бы позвать судью к вопросу, на который уже ответили.
...
# Минимальные размеры недельной выборки, при которых граница вообще что-то
# значит (#1145 ревью, отчёты 203 и 214). У КАЖДОЙ границы свой минимум, и
# это не аккуратность: один минимум, выведенный из пола, глушил потолок —
# судья, эскалировавший пятнадцать из пятнадцати, читался как «не измерен»,
# а не как «бесполезен» (отчёт 214).
#
# Правило одно на обе стороны: граница значима с того n, при котором её
# можно нарушить НЕВЫРОЖДЕННЫМ счётом. Для пола — одна эскалация уже строго
# ниже: 1/n < f, то есть n > 1/f. Для потолка — одна НЕ-эскалация уже строго
# выше: (n-1)/n > c, то есть n > 1/(1-c). Меньше — и «ниже пола» означает
# ровно «ни одной эскалации», а «выше потолка» — ровно «все эскалации»: обе
# измеряют размер выборки, а не судью.
#
# Числа не выбраны, а ВЫВЕДЕНЫ: floor(1/x) + 1 есть наименьшее целое строго
# больше 1/x. Первый заход брал ceil(1/f) = 20, и при n = 20 одна эскалация
# даёт ровно 5% — не ниже пола, а на нём; «ниже» всё ещё означало «ноль».
# Ошибка на единицу, найденная перечитыванием собственного вывода.
CORRIDOR_MIN_FOR_FLOOR = math.floor(1 / ACT_ESCALATION_FLOOR) + 1
CORRIDOR_MIN_FOR_CEILING = math.floor(1 / (1 - ACT_ESCALATION_CEILING)) + 1
...
class WeeklySample:
    ...
    @property
    def floor_measurable(self) -> bool:
        return self.judged >= CORRIDOR_MIN_FOR_FLOOR

    @property
    def ceiling_measurable(self) -> bool:
        return self.judged >= CORRIDOR_MIN_FOR_CEILING
...
def corridor_state(sample: WeeklySample) -> tuple[str, str]:
    ...
    согласие ничего не стоит.

    Границы проверяются НЕЗАВИСИМО, каждая от своего минимума: потолок
    значим с трёх суждений, пол — с двадцати одного. Иначе минимум,
    выведенный из пола, глушил бы потолок (отчёт 214): пятнадцать
    эскалаций из пятнадцати — это «бесполезен», а не «не измерено».

    «Не измерено» — одно состояние на три причины: пустая неделя, неделя
    короче любого минимума, и неделя, где потолок проверен и не нарушен, а
    пол ещё не значим. Текст называет, какая именно. Подставить туда долю
    значило бы обвинить молчание в штамповке (#762) — или первое суждение,
    потому что 0 из 1 это 0%.

    Текст несёт счётчик и долю с десятыми: округление до целого выдавало
    «5% ниже 5%» на 1 из 21. И текст пола различает «ни разу» и «редко»:
    судье, эскалировавшему один раз из двадцати одного, «не пробовал не
    согласиться» — неправда (отчёт 214).
    """
    n = sample.judged
    if n == 0:
        return (
    ...
    if sample.ceiling_measurable and share > ACT_ESCALATION_CEILING:
        return (
            REASON_OVER_ESCALATING,
    ...
    if sample.floor_measurable and share < ACT_ESCALATION_FLOOR:
        why = (
            "он не пробовал не согласиться ни разу"
            if sample.escalated == 0
            else "несогласие у него — редкость, а не позиция"
        )
        return (
            REASON_STAMPING,
            f"эскалаций за неделю {seen} — ниже {ACT_ESCALATION_FLOOR:.0%}: "
            f"стюард соглашается почти со всем подряд — {why}, и его согласие "
            "сейчас ничего не стоит",
        )
    if not sample.floor_measurable:
        ceiling_note = (
            f"верхняя граница проверена по {n} и не нарушена"
            if sample.ceiling_measurable
            else f"верхняя граница значима от {CORRIDOR_MIN_FOR_CEILING}"
        )
        return (
            CORRIDOR_NO_SAMPLE,
            f"за последние {CORRIDOR_WINDOW_DAYS} дней суждений {n}: нижняя "
            f"граница значима от {CORRIDOR_MIN_FOR_FLOOR}, {ceiling_note}; "
            "меньше — и «ниже пола» измеряет размер выборки, а не судью",
        )
    return (
        CORRIDOR_INSIDE,
...
async def _announce_corridor_state_once(
    ...
    await repo.insert_event(
        ...
        payload={
            "state": state,
            # Доля пишется только там, где она измерена. Недостаточная
            # неделя — состояние no_sample, и доля в нём читалась бы как
            # результат: пустота не чистота (#762, отчёт 214). Счётчики
            # остаются — они факты, а не вывод.
            "share": sample.share if state != CORRIDOR_NO_SAMPLE else None,
