...
#:
#: ИЗМЕРЕНО, а не выбрано: прогон правила по окнам 2..5 на размеченных
#: траекториях (tests/test_review_dispatch.py::NON_CONVERGENCE_HISTORY).
#: Окно 2 останавливает сошедшуюся #1204 [6,2,2,2,…] на плато из трёх
#: двоек — ложная остановка. Окно 4 и шире пропускает все три короткие
#: несошедшиеся (#1081 [5,6,6], #1084 [3,6,3], #1186 [7,2,13]). Окно 3 —
#: единственное, при котором сошедшихся остановлено зря НОЛЬ, а поймано
#: всё, #1208 — на четвёртом поколении. Тест
...
def _fires_at_last(counts: Sequence[int], window: int) -> bool:
    ...
    Условие стоит, когда отчётов не меньше ``window``, на последнем открытые
    находки есть и верно одно из двух:

    * находок не меньше, чем в ПЕРВОМ отчёте: после window−1 кругов правок
      задача не ушла ниже старта (#1081, #1084, #1186, #1208);
    * наименьший счёт не обновлялся ``window`` отчётов подряд — колебание,
      не опускающееся ниже уже достигнутого.

    Ноль — схождение, на нём условие не стоит никогда.
    """
    if len(counts) < window or counts[-1] == 0:
        return False
    best_at = min(range(len(counts)), key=lambda i: (counts[i], i))
    return counts[-1] >= counts[0] or len(counts) - 1 - best_at >= window
