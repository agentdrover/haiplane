...
Отдельного образа в registry нет: команда выше собирает его из этого клона.
Python и uv в `Dockerfile` закреплены digest'ом, поэтому `--build` поднимает
текущий хаб, а не плавающий `latest`, оставшийся от прошлой сборки.
