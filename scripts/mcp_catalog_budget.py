...
def _write_budget(
    ...
    doc = {
        ...
        "working_headroom_note": (
            ...
            "одного ключа."
            "\n\nПОЛИТИКА ПОПОЛНЕНИЯ, КОГДА ЗАПАСА НЕТ (#1241). 2026-09-09 "
            "три задачи в один день ответили на этот вопрос по-разному: "
            "#1155 подняла budgets.schema_chars правкой файла; #1236 не "
            "подняла ничего, а подрезала чужие описания (hub_prepare_"
            "developer_task, hub_refine_task) для чужой работы; #1215 "
            "убрала готовый инструмент из сдачи вовсе, без названной цены. "
            "Порядок один и записан здесь, а не в трёх местах: (1) сначала "
            "платите подрезкой в ЭТОМ ЖЕ диффе — дешевле и не требует "
            "решения; (2) если подрезки не хватает, потолок поднимает "
            "ЧЕЛОВЕК (владелец или ревьюер задачи) явным решением, а причина "
            "уходит в _comment этого файла рядом с поднятым числом, как у "
            "#1155; (3) если до конца работы ни (1), ни (2) не приняты, "
            "работа откладывается, а не выбрасывается тихо — отказ называет "
            "недостающее число знаков в task update. Агент не поднимает "
            "budgets сам: подъём — это шаг (2), и он требует записанного "
            "решения человека, а не отдельного вызова --update."
        ),
        "baseline_tools": {
