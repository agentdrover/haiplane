...
PRE_TRIM_CEILINGS = {
    ...
    # #1155: единственный поднятый потолок, 31390 → 34716, и он назван здесь,
    # чтобы храповик остался храповиком для остальных четырёх. Запас под ним
    # был исчерпан до 5 символов, то есть проверка краснела бы от ЛЮБОГО нового
    # параметра любого инструмента — состояние, в котором она перестаёт
    # различать рост и мелочь. Новое число — правило самого файла: живой
    # каталог плюс объявленные 10%. Рабочая заморозка (WORKING_FREEZE) при
    # этом не двигалась: новое описание оплачено подрезкой, как и положено.
    "schema_chars": 34716,
