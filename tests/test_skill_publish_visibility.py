...
# через одну. Замер ДО починки на нём (CPython 3.12.13, интерпретатор проекта):
# 1000 строк — 0.04 с, 6000 (104 КБ) — 1.54 с, 16000 (286 КБ) — 10.67 с, и это
# только сводка; страница зовёт следом ``unified_diff``, то есть вдвое больше.
# Вход именно такой, а не случайно перемешанный: на перемешанном тормозов нет
# вовсе — при уникальных строках срабатывает эвристика autojunk из difflib.
...
def test_an_ordinary_edit_in_a_big_skill_is_still_counted():
    ...
    поэтому от файла в 20000 строк (634 КБ) остаётся почти пустая задача
    (замер: 250 500 клеток при потолке 4 000 000, 0.03 с на оба вызова). Без
    этого теста потолок можно было бы опустить до нуля и объявить «починено» —
    при том, что диф исчез бы вообще везде.
    """
