...
@pytest.mark.asyncio
async def test_renew_moves_the_deadline_but_not_onto_a_spent_code(hub, monkeypatch):
    """#1208: продление отмеряет срок заново — и только НЕПОТРАЧЕННОМУ коду.

    Продление нужно там, где между чеканкой кода и его первым употреблением
    стоит ожидание, которого чеканящий не видит: локальный прогон ревью
    чеканит код внутри HTTP-запроса автора, а слот хоста берёт уже в фоне,
    простояв в очереди дольше, чем живёт код.

    Три следствия разом, потому что каждое из них — отдельный способ сделать
    продление вредным. Протухший код оживает: иначе продление не лечит ровно
    тот случай, ради которого написано. Потраченный НЕ оживает: продлённый,
    он дал бы вторую сессию по одному предъявлению. И ответ правдив — по нему
    вызывающий решает, поедет ли отчёт слабым путём.
    """
    monkeypatch.setattr(config, "CHAT_PAIR_CODE_SECONDS", 1)
    task_id = await _in_review(hub, "простоял в очереди")
    code, _ttl = await cp.issue_code(
        hub.db, hub.human_id, kind="reviewer", bound_task_id=task_id, bound_generation=1
    )
    # Срок уже истёк — ровно то, с чем просыпается второй в очереди.
    await hub.db.execute(
        "UPDATE chat_pair_codes SET expires_at = datetime('now', '-60 seconds')"
    )
    await hub.db.commit()
    resp = await hub.client.post(
        "/api/auth/chat-pair/redeem", json={"code": code}, headers=_ip()
    )
    assert resp.status_code == 401, "протухший код обязан быть мёртв до продления"

    assert await cp.renew_code(hub.db, code) is True, (
        "продление не нашло кода, который лежит в базе непотраченным"
    )
    resp = await hub.client.post(
        "/api/auth/chat-pair/redeem", json={"code": code}, headers=_ip()
    )
    ...
    assert await cp.renew_code(hub.db, code) is False, (
        "потраченный код продлён: одно предъявление дало бы вторую сессию"
    )
